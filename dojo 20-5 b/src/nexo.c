...
{
	printf("%4d %10s %4.2f %d %s\n", miAlumno.legajo, miAlumno.nombre, miAlumno.promedio, miAlumno.idCurso, miCurso.descripcion);
}

void MostrarListaDeAlumnos(eAlumno listaAlumnos[], int sizeAlumnos, eCurso listaCursos[], int sizeCursos)
{
	for(int i=0; i<sizeAlumnos; i++)
	{
		for(int k = 0; k < sizeCursos; k++)
		{
			if(listaCursos[k].idCurso == listaAlumnos[i].idCurso &&  listaAlumnos[i].estado == OCUPADO)
			{
				MostrarUnAlumno(listaAlumnos[i], listaCursos[k]);
			}
		}
	}
}
